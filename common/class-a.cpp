...
int A::throw_in_ctor_for_seq_no_{-2}; // note: `-1` is for moved instance with
                                      // its undefined state
...
void A::_init(void) {
    ...
    // This method will be used as an initializer in A::ctor

    // Note that every time the constructor throws, we have a memory leak in
    // `raw_string_`
}
