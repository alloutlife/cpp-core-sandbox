...
int main(void) {
    ...
    // Although we define `ref_rval` as a reference to rvalue, the compiler will
    // avoid calling && variant of overloaded functions because this rvalue
    // reference object has identifier. Normally you see such a definition in
    // function arguments, not on local stack

    std::string &&ref_rval = std::move(inst1); // the entry looks weird
    [[maybe_unused]] const std::string &&ref_rval_const =
        std::move(inst1); // The code is valid, although doesn't make a sense
    ...
    std::string inst3{
        ref_rval}; // still calls string( const string& )
                   // From the perspective of compiler `ref_rval` still lives
                   // after call, So it doesn't call && constructor
    ...
    move_or_copy::fn1(ref_rval); // The same result. Doesn't call && variant.
                                 // In this context the variable has name, so
                                 // it's treated as an lvalue
    ...
    // Step aside. An isolated simple example
    {
        ...
        std::string s4 =
            s1 + s2; // s1 + s2 results in creating a prvalue. We are not likely
                     // to observe a && construction here most likely the
                     // compiler will elide copy.

        std::string s5{std::move(s4)}; // std::move returns a reference to
                                       // rvalue that isn't bound to a name

        std::cout << "s4: " << s4 << std::endl;
        std::cout << "s5: " << s5 << std::endl;
    }

    // TODO: how to pass an initialization value via function call: const ref,
    // &&, pass by value? Pros, cons
