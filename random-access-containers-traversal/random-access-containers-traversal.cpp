...
int main()
{
    // We have two random access containers in STL: `std::vector` and
    // `std::deque`. The following code demonstrates that it doesn't matter
    // which way you traverse a `std::array`. But on the contrary it is
    // preferred to use iterators or `range-based for` in case of `std::deque`
    // traversal
    //
    // I also found that the code compiled with GCC 11.2 performs slightly
    // better than compiled with Clang 13.0
    ...
    container_type s1;
    int generator{0};
    std::generate_n(std::back_inserter(s1), array_size,
                    [&generator] { return (++generator) % 256; });
